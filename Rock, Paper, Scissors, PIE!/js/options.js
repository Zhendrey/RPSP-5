...
//let streaksCountData__value = streaksCountData[0].streakCount;
...
/*const dynamicallyAddedOptions = document.querySelectorAll('.dynamic');
function renderAmountsOfImageItems(item){
    const options__itemAmount = document.createElement('div');
    options__itemAmount.classList.add("options__item-amount");

        [...dynamicallyAddedOptions].forEach(dynamic =>{
            dynamic.prepend(options__itemAmount)
        })
...
imageItems.forEach(item =>{
    renderAmountsOfImageItems(item)
})*/
...
//const userIten__INDEX = userItem__INPUT[0].id;
...
function renderBoughItem(itemsParent, bought__item, storeItem, index){
    ...
    storeItem['image'])
    remaining__text.textContent = `${areItemsBought[index]['amount']} remaining`;
    //remaining__text.textContent = `${areItemsBought[index][Object.keys(areItemsBought[index])[0]]} remaining`;
}
...
quotes.onload = ()=>{
    realQuotes = JSON.parse(quotes.responseText) || undefined;
}
/*
quotes.onerror = ()=>{
    inspirational_quote__child.innerHTML = `OOPS! WE COULD NOT LOAD THE QUOTES :(`
    console.log(`OOPS! WE COULD NOT LOAD THE QUOTES :(`);
}
*/
let generatedQuote;
...
function createLossArr(start = 0, end){
    ...
    /*lossArr.push(
    [user__INDEXES[user__INDEXES.length - 2],user__INDEXES[user__INDEXES.length - 2]],
    [user__INDEXES[user__INDEXES.length - 2],user__INDEXES[user__INDEXES.length - 1]],
    [user__INDEXES[user__INDEXES.length - 1],user__INDEXES[user__INDEXES.length - 1]],
    [user__INDEXES[user__INDEXES.length - 1],user__INDEXES[user__INDEXES.length - 2]],
    )*/
...
function tie(){
    ...
    //let allTheQuotes__length = realQuotes[2].allTheQuotes.length;
    generatedQuote = realQuotes[2].allTheQuotes[Math.floor(Math.random() * 20)];
    ...
    inspirational_quote__child.innerHTML = `"${generatedQuote}"`;
        ...
        pies__amount_child_p.textContent = data[0].piesRecieved__perGame;
        streak__amount.textContent = streaksCountData[0].streakCount;
        /*piesRecieved_perGame = 0;
        totalPiesAmount += piesRecieved_perGame;*/
    }
        ...
        if(enemyItem__INDEX >= 3 && data[1].totalPiesAmount !== 0){
            switch (true) {
                case data[1].totalPiesAmount > 100:
                    pies__amount_child_p.textContent = Math.floor(-data[1].totalPiesAmount * 0.1)
                    data[1].totalPiesAmount = Math.ceil(data[1].totalPiesAmount * 0.9)
                    break;
            
                case data[1].totalPiesAmount <= 100:
                    pies__amount_child_p.textContent = Math.floor(-data[1].totalPiesAmount * (1 / 3))
    ...
    generatedQuote = realQuotes[1].negativeQuotes[Math.floor(Math.random() * negativeQuotes__length)];
...
function victory(){
    ...
    generatedQuote = realQuotes[0].positiveQuotes[Math.floor(Math.random() * positiveQuotes__length)];
...
//!CREATING THE BOUGHT ITEM'S FUNCTIONALITY
...
function itemFunctionality(event) {
    ...
    if(targetItem){
        ...
        itemRemaining.textContent = `${amountOfBoughtItems[0]["amount"]} remaining`;
