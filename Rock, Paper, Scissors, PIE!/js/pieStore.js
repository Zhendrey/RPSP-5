    ...
    if(areItemsBought.length !== 0){
        for(let i = 0; i < cardRemaining.length; i++){
            if(amountOfBoughtItems[i][Object.keys(amountOfBoughtItems[i])[0]] === 0){
                //const noRemainigStyle = getComputedStyle(cardRemaining[i]);

                continue;
            }
            cardRemaining[i].textContent = `${amountOfBoughtItems[i][Object.keys(amountOfBoughtItems[i])[0]]} remaining`;
        }
...
function buyItem(){
    ...
    cardRemaining[pickedCard__id].textContent = `${amountOfBoughtItems[pickedCard__id][Object.keys(amountOfBoughtItems[pickedCard__id])[0]]} remaining`;
    piesAmount.textContent = data[1].totalPiesAmount;
    localStorage.setItem("amountOfBoughtItems", JSON.stringify(amountOfBoughtItems));
    localStorage.setItem("piesAmount", JSON.stringify(data));


    doubleCheck__bought.style.zIndex = '1';
    setTimeout(()=>{
    doubleCheck__bought.classList.remove('double-check__bought_active')
    }, 1000)
    setTimeout(() =>{
    doubleCheck__bought.style.zIndex = '-2';
...
/*enoughPies__item = Array.from(card).filter(card =>{
    return !card.classList.contains("not-enough")
})
console.log(enoughPies__item);*/
